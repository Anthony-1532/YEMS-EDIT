...
import { User as UserIcon, Key, ShieldCheck, Settings } from 'lucide-react';
            ...
            {[
              {
                ...
                badge: <Badge tone="success" dot>Active</Badge>,
              },
              {
                label: 'Email Verified',
                desc: user?.emailVerified ? 'Your email is verified' : 'Email not yet verified',
                badge: <Badge tone={user?.emailVerified ? 'success' : 'warning'} dot>{user?.emailVerified ? 'Verified' : 'Unverified'}</Badge>,
              },
              {
                label: 'Role',
                desc: 'Your system role and permissions level',
                badge: <Badge tone="maroon">{user?.role}</Badge>,
              },
            ].map((item, i, arr) => (
        ...
        <Card>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between py-2 border-b border-border/50">
              ...
              <p className="text-sm font-medium text-foreground">Yeshua EMS</p>
            </div>
